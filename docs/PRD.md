...
**MVP 阶段输入框只识别 `#tag` 和 `/log` `/done` 行首命令**。其他输入语法（如 `@time` 截止日期、行尾 `!`/`!!`/`!!!` 优先级）属于 database 能力的 Level 2 演进，详见独立的 database 设计文档；引入时机以用户行为信号触发，不属于 MVP。

**输入辅助工具栏（未来引入）**：当 Level 2 引入 `@`、`!` 等需要切换输入法的符号时，可在输入框正上方提供一个小工具栏，让用户点击插入 `#` `@` `!` 等符号到光标位置。这不是新规则，而是已有规则的辅助输入，类似浏览器地址栏上方的 `.com` 快捷输入。工具栏在桌面端可由用户在设置中关闭，移动端默认显示。
...
- 仅 `todo` 状态触发通知；`done` / `log` 永不触发
- 同一个 entry 的同一个 due 时间只通知一次，避免重复打扰
- 用户勾选完成、删除 todo、清除 due 后，不再触发该通知
...
- 输入框 MVP 不解析自然语言时间；`@time` 等输入语法仍属于 database Level 2 演进

**系统行为**：

- 桌面端使用操作系统通知能力；移动端使用系统本地通知能力
- 系统通知权限未授权时，App 应在设置或首次设置 due 时引导用户授权；未授权不阻塞创建 todo
- App 启动、vault reload、同步完成、entry 创建/修改/完成/删除后，都必须重建或更新待通知队列
...
`due` 是首版识别的用户字段，用于 todo 截止时间与系统通知（§4.9）。格式为 ISO 8601 datetime；如用户只选择日期，App 应按本地时区保存为当天 23:59。其他用户字段在 MVP 不提供专用编辑 UI，但解析器、MCP `set_entry_property` tool、AI 必须能读写这些字段。这是为后期 database 能力埋的基础。
...
3. **哲学**：人生本身由每天、每周、每季度、每年的循环组成。Loop 不是隐喻，而是产品要承载的事实。
...
- **圆环**代表循环、系统、轨道。
- **缺口**代表尚未完成的 open loop，是 todo app 存在的理由。
- **橙点**代表下一步行动（next action），停在缺口起点，等待被推进以闭合循环。

产品名 Loop 指向闭合的理想状态，而 logo 呈现未闭合的现实状态。中间的缺口就是产品和 AI 要帮助用户完成的工作：把开的变成闭的。
...
Logo 必须能被参数化复现，便于缩放、动效和多尺寸资产生成。图标缺口方向（12 点 → 9 点）与字母 **L** 的形状呼应。
...
一句话总结：**Loop 是一个未闭合的圆，等着一个橙点把它合上。**
...
- [ ] 完成、删除或清除 due 后不再触发到期通知
- [ ] 长按/右键操作菜单完整
- [ ] 滚动加载更早条目流畅

**二级页**：
- [ ] 详情页编辑 content 后正确解析标签
- [ ] 详情页可设置、修改、清除 todo 截止时间
...
- **决策**：任何新增的输入语法、交互规则、命令前缀、UI 控件，必须经过价值非冗余 + 协调一致双重检查；规则数量不是问题，规则之间冲突才是问题
- **理由**：极简产品的复杂度增长是隐性的，每条新规则单看都"合理"，但叠加后会破坏整体协调性；明确的检查标准比"是否极简"的主观判断更可靠
- **替代**：
  - 一律拒绝新规则——过度限制，连有价值的规则也无法引入
  - 凭直觉判断——主观且不可传承
- **实际拒绝过的设计**（讨论中产生）：
